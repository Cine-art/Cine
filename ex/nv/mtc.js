...
function getManifest() {
    return JSON.stringify({
        ...
        "name": "MeTruyenChuVN",
        "version": "1.0.0",
        "baseUrl": "https://metruyenchuvn.com",
        "iconUrl": "https://metruyenchuvn.com/images/logo.png",
        "isEnabled": true,
        "type": "NOVEL"
    });
}
...
function getUrlList(slug, filtersJson) {
    try {
        ...
        if (page > 1) {
            return "https://metruyenchuvn.com/" + categorySlug + "?page=" + page + globalUaSuffix;
        }
        return "https://metruyenchuvn.com/" + categorySlug + globalUaSuffix;
    } catch (e) {
        return "https://metruyenchuvn.com/danh-sach/truyen-hot" + globalUaSuffix;
    }
}
...
function getUrlSearch(keyword, filtersJson) {
    try {
        ...
        var page = filters.page || 1;
        if (page > 1) {
            return "https://metruyenchuvn.com/search?q=" + encodeURIComponent(keyword) + "&page=" + page + globalUaSuffix;
        }
        return "https://metruyenchuvn.com/search?q=" + encodeURIComponent(keyword) + globalUaSuffix;
    } catch (e) {
        return "https://metruyenchuvn.com/search?q=" + encodeURIComponent(keyword) + globalUaSuffix;
    }
}
...
function getUrlDetail(slug) {
    ...
    if (slug.indexOf("http://") === 0 || slug.indexOf("https://") === 0) {
        ...
        url = "https://metruyenchuvn.com" + slug;
    } else {
        url = "https://metruyenchuvn.com/" + slug;
    }
...
function parseListResponse(htmlContent) {
    var items = [];
    htmlContent = htmlContent.replace(/class='item'/g, 'class="item"');
    ...
    for (var i = 1; i < blocks.length; i++) {
        ...
        if (cover && cover.indexOf("http") !== 0) {
            cover = "https://metruyenchuvn.com" + cover;
        }
...
function parseMovieDetail(htmlContent, apiUrl) {
    ...
    if (imgMatch) {
        ...
        if (posterUrl && posterUrl.indexOf("http") !== 0) {
            posterUrl = "https://metruyenchuvn.com" + posterUrl;
        }
    }
    ...
    if (chapListMatch) {
        ...
        while ((chapMatch = liRegex.exec(chapListMatch[1])) !== null) {
            ...
            if (chapUrl.indexOf("/") === 0) {
                chapUrl = "https://metruyenchuvn.com" + chapUrl;
    ...
    try {
        ...
        if (maxPage > 1) {
            var bidMatch = /page\((\d+),\d+\)/i.exec(htmlContent);
            if (bidMatch) {
                var bid = bidMatch[1];
                var fetchLimit = maxPage > 20 ? 20 : maxPage;
                
                for (var p = 2; p <= fetchLimit; p++) {
                    var pageUrl = "https://metruyenchuvn.com/get/listchap/" + bid + "?page=" + p;
                    ...
                    if (pageHtml) {
                        try {
                            var jsonResp = JSON.parse(pageHtml);
                            if (jsonResp && jsonResp.data) {
                                pageHtml = jsonResp.data;
                            }
                        } catch(e) {}
                        ...
                        while ((pChapMatch = pLiRegex.exec(pageHtml)) !== null) {
                            ...
                            if (pChapUrl.indexOf("/") === 0) {
                                pChapUrl = "https://metruyenchuvn.com" + pChapUrl;
